...
import (
	...
	"ekko/pkg/carbon/pkg/tls"
	api "ekko/pkg/redis/api"
)
...
func New(config *api.Redis, opts ...Option) (*redis.Client, error) {
	...
	if config.GetContextTimeoutEnabled() {
		o.ContextTimeoutEnabled = config.GetContextTimeoutEnabled()
	}
	...
	if config.GetMaxIdleConns() != 0 {
		o.MaxIdleConns = int(config.GetMaxIdleConns())
	}
	if config.GetConnMaxIdleTime() != 0 {
		o.ConnMaxIdleTime = time.Duration(config.GetConnMaxIdleTime()) * time.Millisecond
	}
	if config.GetConnMaxLifetime() != 0 {
		o.ConnMaxLifetime = time.Duration(config.GetConnMaxLifetime()) * time.Millisecond
	}
	if config.GetTls() != nil && config.GetTls().GetEnabled() {
		tlsConfig, err := tls.New(config.GetTls())
		if err != nil {
			return nil, err
		}
