...
type Flags struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	EnableRedis   bool                   `protobuf:"varint,1,opt,name=enable_redis,json=enableRedis,proto3" json:"enable_redis,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Flags) Reset() {
	*x = Flags{}
	mi := &file_ekko_api_ekko_config_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}
...
func (*Flags) Descriptor() ([]byte, []int) {
	return file_ekko_api_ekko_config_proto_rawDescGZIP(), []int{3}
}

func (x *Flags) GetEnableRedis() bool {
	if x != nil {
		return x.EnableRedis
	}
	return false
}
...
const file_ekko_api_ekko_config_proto_rawDesc = "" +
	"\n" +
	"\x1aekko/api/ekko_config.proto\x12\vekko.config\x1a\x17validate/validate.proto\x1a\x17logger/api/logger.proto\x1a\x17carbon/api/carbon.proto\x1a\x15redis/api/redis.proto\x1a\x1bdatabase/api/database.proto\"\xd7\x03\n" +
	"\x06Config\x12?\n" +
	...
	"\tbulbasaur\x18\x06 \x01(\v2\x1c.ekko.config.InternalServiceR\tbulbasaur\x12(\n" +
	"\x05flags\x18\a \x01(\v2\x12.ekko.config.FlagsR\x05flags\"\xfc\x01\n" +
	"\bRabbitMQ\x12\x18\n" +
	...
	"\x05Flags\x12!\n" +
	"\fenable_redis\x18\x01 \x01(\bR\venableRedisB\x16Z\x14ekko/pkg/config;ekkob\x06proto3"
...
var file_ekko_api_ekko_config_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_ekko_api_ekko_config_proto_goTypes = []any{
	...
	(*Flags)(nil),           // 3: ekko.config.Flags
	(*api.Listener)(nil),    // 4: greyhole.carbon.Listener
	(*api1.Logger)(nil),     // 5: greyhole.logger.Logger
	(*api2.Database)(nil),   // 6: greyhole.database.Database
	(*api3.Redis)(nil),      // 7: greyhole.redis.Redis
}
var file_ekko_api_ekko_config_proto_depIdxs = []int32{
	4, // 0: ekko.config.Config.listener:type_name -> greyhole.carbon.Listener
	5, // 1: ekko.config.Config.logger:type_name -> greyhole.logger.Logger
	6, // 2: ekko.config.Config.database:type_name -> greyhole.database.Database
	7, // 3: ekko.config.Config.redis:type_name -> greyhole.redis.Redis
	4, // 4: ekko.config.Config.http_listener:type_name -> greyhole.carbon.Listener
	...
	3, // 7: ekko.config.Config.flags:type_name -> ekko.config.Flags
